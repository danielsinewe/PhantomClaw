...
ALTER TABLE automation_daily_metrics ADD COLUMN IF NOT EXISTS snapshot_id BIGSERIAL;
ALTER TABLE automation_daily_metrics DROP CONSTRAINT IF EXISTS automation_daily_metrics_pkey;
CREATE INDEX IF NOT EXISTS automation_daily_metrics_lookup_idx
  ON automation_daily_metrics (workspace_slug, platform, profile_name, metric_name, captured_at);
"""

NORTH_STAR_DAILY_METRICS_VIEW_SCHEMA = """
DROP VIEW IF EXISTS automation_daily_metric_daily_latest_v1;
...
SELECT
  ...
  LAG(metric_value) OVER (
    ...
    ORDER BY captured_at_ts, snapshot_id
...
CREATE OR REPLACE VIEW automation_daily_metric_daily_latest_v1 AS
SELECT DISTINCT ON (workspace_slug, platform, profile_name, metric_name, metric_date)
  snapshot_id,
  workspace_slug,
  platform,
  profile_name,
  ...
  run_id,
  metadata_json
FROM automation_daily_metrics_v1
ORDER BY workspace_slug, platform, profile_name, metric_name, metric_date, captured_at_ts DESC, snapshot_id DESC;
"""
...
SELECT
  ...
  CASE
    WHEN metrics_json ? 'peerlist_profile_following_before'
      AND metrics_json->>'peerlist_profile_following_before' <> ''
    THEN (metrics_json->>'peerlist_profile_following_before')::integer
    ELSE NULL
  END AS peerlist_profile_following_before,
  CASE
    WHEN metrics_json ? 'peerlist_profile_following_after'
      AND metrics_json->>'peerlist_profile_following_after' <> ''
    THEN (metrics_json->>'peerlist_profile_following_after')::integer
    ELSE NULL
  END AS peerlist_profile_following_after,
  CASE
    WHEN metrics_json ? 'peerlist_profile_following_delta'
      AND metrics_json->>'peerlist_profile_following_delta' <> ''
    THEN (metrics_json->>'peerlist_profile_following_delta')::integer
    ELSE NULL
  END AS peerlist_profile_following_delta,
  CASE
...
def peerlist_follow_workflow_metrics(report: Any) -> dict[str, Any]:
    ...
    following_before = data.get("peerlist_profile_following_before")
    following_after = data.get("peerlist_profile_following_after")
    following_delta = data.get("peerlist_profile_following_delta")
    if following_delta is None and following_before is not None and following_after is not None:
        following_delta = int(following_after) - int(following_before)
    ...
    return {
        ...
        "metrics_json": {
            ...
            "peerlist_profile_following_delta": following_delta,
