...
DEFAULT_ARTIFACT_DIR = Path("artifacts/linkedin-sales-community")
...
def parse_args(argv: list[str] | None = None):
    ...
    parser.add_argument("--url", default=os.getenv("LINKEDIN_SALES_COMMUNITY_URL", DEFAULT_URL))
    parser.add_argument("--chrome-profile", default=os.getenv("LINKEDIN_SALES_COMMUNITY_PROFILE"))
    parser.add_argument("--session-name", default=os.getenv("LINKEDIN_SALES_COMMUNITY_SESSION", "linkedin-sales-community"))
    parser.add_argument("--artifact-dir", type=Path, default=DEFAULT_ARTIFACT_DIR)
    parser.add_argument("--db-path", type=Path, default=DEFAULT_DB_PATH)
    parser.add_argument("--like-cap", type=int, default=int(os.getenv("LINKEDIN_SALES_COMMUNITY_LIKE_CAP", "1")))
    parser.add_argument(
        "--analytics-database-url",
        default=os.getenv("LINKEDIN_SALES_COMMUNITY_ANALYTICS_DATABASE_URL") or os.getenv("AUTOMATION_ANALYTICS_DATABASE_URL"),
    )
    parser.add_argument("--dry-run", action="store_true")
    args = parser.parse_args(argv)
    if not args.dry_run and not args.chrome_profile:
        raise SystemExit("Missing required configuration: LINKEDIN_SALES_COMMUNITY_PROFILE")
    return args
